...
Workbook slice relevant for **Värme**. The `Värme` sheet carries the per-building accounting formulas; STRUX is the meter catalog; Avläsning/PME/PME_EL are the underlying reading sources that the accounting formulas XLOOKUP into.
...
| Värme | 71 | 45 |  | heating per-building accounting |
